...
- `SessionBootstrap` による XR session lifecycle（start/end/visibility/reference space）管理と flat fallback
...
- combat sandbox (`CombatSandboxDirector`)
  ...
  - arena device runtime effect（hazard / focus drain / guard tax / overburn pressure）を導入し、phase hazard と合成した環境圧をデータ駆動で適用
