...
export class CombatSandboxDirector {
  private readonly enemies: SandboxEnemy[] = [
    this.createEnemy('ash-mote', 'Ash Mote', 30, 8, 2.4),
    this.createEnemy('candle-penitent', 'Candle Penitent', 42, 10, 3.2),
    this.createEnemy('furnace-thurifer', 'Furnace Thurifer', 60, 14, 4.6)
  ];
  ...
  private objective = 'Defeat the combat sandbox wave in Pilgrim’s Belfry.';
  ...
  private rotateLoadout(): void {
    this.weaponIndex = (this.weaponIndex + 1) % WEAPON_DEFS.length;
    this.offhandIndex = (this.offhandIndex + 1) % OFFHAND_DEFS.length;
    this.objective = `Loadout changed: ${WEAPON_DEFS[this.weaponIndex].displayName} + ${OFFHAND_DEFS[this.offhandIndex].displayName}.`;
  }
